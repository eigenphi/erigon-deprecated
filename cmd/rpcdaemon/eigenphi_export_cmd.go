...
func GetExportCmd(cfg *httpcfg.HttpCfg, ctx context.Context, rootCancel context.CancelFunc) *cobra.Command {
	var (
		outJsonL    bool
		outProtobuf bool
		outputDir   string
...
./rpcdaemon export block 122 222 ( export transaction from height: 122 to height: 222)`,
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			ctx := cmd.Context()
			db, borDb, _, _, _, stateCache, blockReader, ff, agg, err := cli.RemoteServices(ctx, *cfg, logger, rootCancel)
			if err != nil {
			...
			base := commands.NewBaseApi(ff, stateCache, blockReader, agg, cfg.WithDatadir, cfg.EvmCallTimeout, engine)
...
./rpcdaemon export simple_transaction 122 222 ( export transaction from height: 122 to height: 222)`,
	...
	exportBlockParquet := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			engine := ethash.NewFaker()
			base := commands.NewBaseApi(ff, stateCache, blockReader, agg, cfg.WithDatadir, cfg.EvmCallTimeout, engine)
			ethImpl := commands.NewEthAPI(base, db, backend, txPool, mining, cfg.Gascap, cfg.ReturnDataLimit)
			...
			if err := commands.ExportParquet(path, data); err != nil {
				zap.L().Sugar().Errorf("export parquet %s", err)
			}
		},
	}

	saveStreamToFile := func(height int64, outputDir string, data []commands.ExportTraceParquet) error {
		var err error
		...
		defer tmpf.Close()
		if err := commands.ExportParquetWithData(tmpf, data); err != nil {
	...
	saveStreamToOSS := func(height int64, outputDir string, data []commands.ExportTraceParquet) error {
		...
		if err := commands.ExportParquetWithData(buffer, data); err != nil {
			return fmt.Errorf("export parquet failed: %s", err)
		}
	...
	exportBlockParquetRun := func(saveFunc func(height int64, outputDir string, data []commands.ExportTraceParquet) error) func(cmd *cobra.Command, args []string) {
		return func(cmd *cobra.Command, args []string) {
			...
			// TODO: Replace with correct consensus Engine
			engine := ethash.NewFaker()
			baseApi := commands.NewBaseApi(ff, stateCache, blockReader, agg, cfg.WithDatadir, cfg.EvmCallTimeout, engine)

			debugImpl := commands.NewPrivateDebugAPI(baseApi, db, cfg.Gascap)
			...
			eg := sync.WaitGroup{}
			ethImpl := commands.NewEthAPI(baseApi, db, backend, txPool, mining, cfg.Gascap, cfg.ReturnDataLimit)
			...
			go func() {
				...
				for traces := range parquetTasks {
					...
					go func(traces parquetTask) {
						...
						var data = make([]commands.ExportTraceParquet, len(traces.traces))
