...
func marshalReceipts(receipts types.Receipts, txns []types.Transaction, chainConfig *params.ChainConfig, block *types.Block) []map[string]interface{} {
	mrs := make([]map[string]interface{}, 0, len(receipts))
	for i, receipt := range receipts {
		txn := txns[i]
		mr := marshalReceipt(receipt, txn, chainConfig, block, txn.Hash())
		mrs[i] = mr
	}
	return mrs
}
